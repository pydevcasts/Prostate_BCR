...
### 🔹 ۷. پیدا کردن پرتکرارترین مقدارها (Mode)

در برخی تحلیل‌ها لازم است بدانیم **کدام مقدار در یک ستون بیشتر از بقیه تکرار شده است**.  
تابع `mode` در پانداس، **پرتکرارترین مقدار یا مقدارها** را در هر ستون برمی‌گرداند.

نکات مهم:
* برای داده‌های عددی و متنی کاربرد دارد.
* اگر چند مقدار فراوانی یکسان داشته باشند، همگی برگردانده می‌شوند.
* خروجی معمولاً یک DataFrame شامل تمام مقدارهای پرتکرار است.
* هر ستون به‌طور جداگانه بررسی می‌شود.
...
توضیح:
* در ستون A دو مقدار ۲ و ۳ بیشترین تکرار را دارند.
* در ستون B مقدارهای x و z پرتکرارترین هستند.

---

### دریافت مد یک ستون مشخص

```python
df["A"].mode()
```

### دریافت فقط اولین مقدار پرتکرار
