...
def transform_selected(
    ...
    Args:
        X: New DataFrame to transform (genes + clinical)
        fitted_selector: Fitted selector from run_feature_selection
        
    Returns:
        DataFrame with selected genes + clinical features
    """
    X = X.copy()
    ...
    # Separate genes and clinical in new data
    all_cols = set(X.columns)
    gene_cols = [c for c in all_cols if c not in clinical_cols]
    
    logger.info(f"Transform: {len(gene_cols)} genes, {len(clinical_cols)} clinical cols")
    
    # Extract gene columns only
    X_genes = X[gene_cols].copy()
    
    # Get variance-filtered columns from fit time
    var_cols = (vt.get_feature_names_out().tolist() 
                if hasattr(vt, 'get_feature_names_out') 
                else list(imputer.feature_names_in_))
    
    # Ensure all variance-filtered columns exist (fill missing with NaN)
    for c in var_cols:
        if c not in X_genes.columns:
            X_genes[c] = np.nan
    
    # Reorder to match fit time exactly
    X_genes = X_genes[var_cols]
    
    # Impute genes using fitted imputer
    X_genes_imp = pd.DataFrame(
        imputer.transform(X_genes),
        columns=var_cols,
        index=X.index
    )
    ...
    X_genes_selected = X_genes_imp[available_genes]
    
    logger.info(f"Transform: {len(available_genes)}/{len(selected_genes)} genes available")
    
    # Get clinical features (ensure they exist)
    available_clinical = [c for c in clinical_cols if c in X.columns]
    X_clinical = X[available_clinical].copy()
    
    logger.info(f"Transform: {len(available_clinical)}/{len(clinical_cols)} clinical cols available")
    
    # Combine selected genes + clinical features
    X_final = pd.concat([X_genes_selected, X_clinical], axis=1)
