...
def estimate_oof_fusion(
    ...
    *,
    ...
    """Estimate fusion weights and threshold from out-of-fold predictions.

    The supplied models should contain the selected hyperparameters, but must
    not be relied on for fitted state. A fresh clone is fitted in each fold,
    so every OOF prediction is generated by a model that did not see that row.
    """
    ...
    for train_idx, valid_idx in splitter.split(X_genomic, y_array):
        genomic_fold_model = clone(genomic_model)
        clinical_fold_model = clone(clinical_model)
        genomic_fold_model.fit(
            _prepare_branch_input(genomic_fold_model, X_genomic.iloc[train_idx]),
            y_array[train_idx],
        )
        clinical_fold_model.fit(
            _prepare_branch_input(clinical_fold_model, X_clinical.iloc[train_idx]),
            y_array[train_idx],
        )
        oof_genomic[valid_idx] = genomic_fold_model.predict_proba(
            _prepare_branch_input(genomic_fold_model, X_genomic.iloc[valid_idx])
        )[:, 1]
        oof_clinical[valid_idx] = clinical_fold_model.predict_proba(
            _prepare_branch_input(clinical_fold_model, X_clinical.iloc[valid_idx])
        )[:, 1]

    if np.isnan(oof_genomic).any() or np.isnan(oof_clinical).any():
        raise RuntimeError("OOF prediction generation left unfilled rows")
