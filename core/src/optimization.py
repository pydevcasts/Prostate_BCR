...
def objective_lightgbm(
    ...
    n_neg = (y_array == 0).sum()
    n_pos = (y_array == 1).sum()
    params["is_unbalance"] = True  # Alternative to scale_pos_weight in LGBM
...
def optimize_model(
    ...
    if model_name == "XGBoost":
        ...
        raise ValueError(f"Unknown model: {model_name}. Supported: XGBoost, LightGBM, CatBoost")
    ...
    # Run optimization
    logger.info(f"Starting Optuna optimization for {model_name}: {n_trials} trials, {cv_splits} CV splits")
...
class VotingEnsemble:
    ...
    def __init__(self, models: Optional[Dict[str, Any]] = None, weights: Optional[List[float]] = None):
        self.models = models or {}
        self.weights = weights
        self.model_names: List[str] = []
    ...
    def fit(self, X_train: pd.DataFrame, y_train: pd.Series | np.ndarray, 
            model_configs: Optional[Dict[str, Dict]] = None) -> "VotingEnsemble":
        """Fit all models in the ensemble."""
        if model_configs is None:
            model_configs = {}
        ...
        for model_name in default_models:
            if model_name not in self.models:
                ...
                try:
                    model = build_model(model_name, y_train=y_train if model_name in ["XGBoost", "LightGBM"] else None, **config)
                    ...
                    logger.warning(f"Failed to train {model_name} for ensemble: {e}")
                    
        self.model_names = list(self.models.keys())
        if self.weights is None and self.model_names:
            self.weights = [1.0 / len(self.model_names)] * len(self.model_names)
            
        logger.info(f"Ensemble trained with {len(self.model_names)} models")
        return self
            ...
            if hasattr(model, 'feature_importances_'):
                ...
                for fname, val in zip(feature_names, imp):
                    if fname not in importances:
                        ...
                        counts[fname] = 0
                    importances[fname] += val
                    counts[fname] += 1
                    
        avg_importance = {k: v / counts[k] for k, v in importances.items()}
        df = pd.DataFrame({
            "feature": list(avg_importance.keys()),
            "importance": list(avg_importance.values())
        })
...
def create_optuna_study(
    ...
    """Create a configured Optuna Study object."""
    if sampler is None:
        sampler = TPESampler(seed=42, multivariate=True)
    if pruner is None:
        pruner = MedianPruner(n_startup_trials=5, n_warmup_steps=10)
...
def analyze_feature_stability(
    ...
    for fold_idx in range(n_folds):
        ...
        for tr_idx, _ in skf.split(X_train, y_train):
            for iter_idx in range(n_iterations):
                try:
                    _, final_features = run_3layer_feature_selection(
                        X_train.iloc[tr_idx], 
                        y_train.iloc[tr_idx],
                    ...
                    logger.warning(f"Feature selection failed in fold {fold_idx}, iter {iter_idx}: {e}")
                    continue
    ...
    logger.info(f"Stability analysis complete: {len(stability_df)} features analyzed")
    return stability_df